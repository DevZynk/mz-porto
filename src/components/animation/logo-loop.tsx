  ...
  ({
    ...
    const renderLogoItem = useCallback(
      (item: LogoItem, key: React.Key) => {
        ...
        if (isNodeItem) {
          const { node, href, title, ariaLabel } = item
          const content = (
            <span
              className={cx(
                ...
                scaleOnHover &&
                  'transition-transform duration-300 ease-in-out group-hover/item:scale-120 hover:text-primary',
              )}
          ...
          const { src, alt, href, title, srcSet, sizes, width, height } = item
          const content = (
            <img
              className={cx(
