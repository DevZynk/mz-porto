...
function getCssVariableColor(element: HTMLElement, variableName: string): [number, number, number] {
  if (typeof window === 'undefined') return [1, 1, 1]
  const tempStyle = element.style.color
  element.style.color = `var(${variableName})`
  const computedColor = window.getComputedStyle(element).color
  element.style.color = tempStyle
  ...
  useEffect(() => {
    ...
    function update(t: number) {
      ...
      if (color[0] === 1 && color[1] === 1 && color[2] === 1) {
        activeColor = getCssVariableColor(container, '--primary')
      }

      program.uniforms.uColor.value.set(...activeColor)
    ...
    return () => {
      ...
      try {
        ...
        // Silently catch node mismatch during unmounting
      }
